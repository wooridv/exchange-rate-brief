...
- **급변 알림(09~18시)**: 아래 두 조건 중 하나라도 충족하면 🚨 별도 잔디 알림(한 건으로 병합).
  - ① **순간 급변**: 직전 10분 대비 매매기준율 **±0.4% 이상** (통화별 **쿨다운 60분**)
  - ② **일중 누적 스윙**: 오늘 시가(오전 9시) 대비 **±0.5% 이상** (방향별 **하루 1회**) — 서서히 크게 움직이는 손해/이익 기회도 포착
  - 장 시작 오버나이트 갭은 제외. 환경변수로 조정: `ALERT_PCT`(0.4), `ALERT_SWING_PCT`(0.5), `ALERT_COOLDOWN_MIN`(60), `ALERT_START_HOUR`(9), `ALERT_END_HOUR`(18).
